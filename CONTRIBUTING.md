...
```bash
git clone https://github.com/PNX89/QUILTZ.git && cd QUILTZ
uv sync --dev
...
uv run mypy
uv run pytest -q
uv run pytest -m helm
uv run scripts/fetch_tools.sh
...
- the chart, with no cluster anywhere
- both binaries against the emulator, and the committed plans
- the queue-to-topic path, which does need a runtime
- the state lock, against a real PostgreSQL
...
https://pnx89.github.io/QUILTZ, come from a real run whose output is committed under
